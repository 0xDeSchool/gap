...
func NewErc1155TransferWatcher(handler Erc1155TransferHandleFunc) *Erc1155Subscription {
	sub := &Erc1155Subscription{
		client:  app.Get[ethclient.Client](),
		handler: handler,
		abi:     erc1155ABI(),
	}
	return sub
	...
	for {
		...
		chanLoop:
			for {
				...
				case vLog := <-logs:
					defer errx.CatchPanic("handle event applog error")
...
func (sub *Erc1155Subscription) parseLog(l *types.Log) []*Erc1155TranferInfo {
	...
	if l.Topics[0] == sub.abi.Events["TransferSingle"].ID {
		...
		if err != nil {
			log.Warn("UnpackLog ERC1155 event applog error")
		}
		data.Raw = *l
		...
		log.Warn("erc1155 event applog topic is not supported: " + l.Topics[0].Hex())
	}
	return result
}
